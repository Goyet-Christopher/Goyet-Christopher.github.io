// GOYET Christopher 
// ORIGINAL : Daniil Arkhangelsky (KikyTokamuro)
// https://www.cssscript.com/segmented-horizontal-bar-graph/
...
const LinerBar = (function() {
    ...
    linerBar.prototype.render = function() {
        ...
        $this.linerBarElements.forEach((element, elementIndex) => {
            ...
            $this.data.items.forEach((item, itemIndex) => {
                ...
                var percent = (item.value / totalValues) * 100;
                progressItem.style.width = `${percent}%`;
                let item_title = createElementWithClass("span", "liner-bar-progess-item-title");
                item_title.textContent = item.title;
                progressItem.appendChild(item_title);
                let item_value = createElementWithClass("span", "liner-bar-progess-item-value");
                item_value.textContent = item.value;
                if (percent>6){
                    item_value.classList.add("labeled");
                }
                progressItem.appendChild(item_value);
    ...
    // const liner_bar_progess_popup_create = function(){
    //     let popup = createElementWithClass('div', 'leaflet-zoom-animated');
    //     let wrapper = this._wrapper = DomUtil.create('div', `${prefix}-content-wrapper`, container);
